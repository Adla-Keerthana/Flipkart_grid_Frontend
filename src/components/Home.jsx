...
const Home = () => {
  ...
  const handleFileChange = (event, module) => {
    setImageFiles({ ...imageFiles, [module]: URL.createObjectURL(event.target.files[0]) });
  };
    ...
    <div className="relative">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        {['expiry', 'label', 'shelfLife', 'brand'].map((module) => (
          <div key={module} className="p-6 bg-gray-800 border border-gray-600 rounded-lg shadow-lg text-center">
            <h2 className="text-2xl font-bold text-neon-yellow mb-4 capitalize">{module.replace(/([A-Z])/g, ' $1')} Module</h2>
            <p className="text-gray-300 mb-4">Use either the camera or file upload to provide images.</p>
            ...
            {imageFiles[module] && (
              <img src={imageFiles[module]} alt={`${module} preview`} className="mb-4 w-full" />
            )}
            {capturedImages[module] && (
              <img src={capturedImages[module]} alt={`${module} capture`} className="mb-4 w-full" />
            )}
      ...
      {showCameraOverlay && (
        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50">
          <div className="relative p-8 bg-gray-900 rounded-lg">
            <h3 className="text-white text-xl mb-4 text-center capitalize">{activeModule} Module - Camera View</h3>

            {/* Video Stream for Camera Capture */}
            <video ref={videoRef} className="w-full mb-4 border border-neon-blue rounded-lg" autoPlay playsInline />
